...
In order to be able to use the syntax, you will need to do these three things:

 - import the syntax for a chosen type class
 - make an instance of that type class available in the implicit scope
 - in some cases the type class instance cannot be provided unless _other_ parts are available in the implicit scope. Most notably this could be an `ExecutionContext` which is required for the creation of type class instances for `Future`.

This may look like a lot of hoops to jump through, but you could mix in the `ApplicativeSyntax` trait into a common shared superclass and import whichever instance you need where you need it. You can also mix in cats type class instances, e.g. `FutureInstances` or `OptionInstances`

Let's say all of your application's controllers are derived from a common base controller class:

{% highlight scala %}
class BaseController extends Controller with ApplicativeSyntax with FutureInstances {
  implicit val ec: ExecutionContext = scala.concurrent.ExecutionContext.Implicits.global
}
{% endhighlight %}

You have all you need in place to do away with all Future.successful's in your controllers:
...
The above code was taken from a Play-framework-bassed application. It has been cut down to omit the names of rendered views and to hide the reverse-routing. It demonstrates usage of the applicative syntax - the two `.pure` that lifted a value into Future context required by the asynchronous play action. As `doSomething` returns a result that's already in the `Future` context, we just map it, no lifting is required. You can also see a bit of cats' `flatMap` syntax (the `ifM` method added onto an `F[Boolean]`) which we will return to discuss later in this article.
