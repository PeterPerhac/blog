...
Both the `Semigroup` and `Monoid` type classes are used for combining a number of values into one. The important detail is that the **combine** operation is _associative_. `Monoid`, which extends `Semigroup`, has additionally the ability to combine 0 values. I will discuss `Monoid`s in this article. We can come up with multiple `Monoid` implementations for single data type. Certain **operations** over a data type demonstrate _monoidal properties_. These operations must be binary (take two operands) and must satisfy these two axioms:
