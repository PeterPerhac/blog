...
description: In this article I will show you a few examples of what you can do by just adding the [cats library][1] to your project's dependencies.
...
This represents the ability to lift a value into a context. (Taking a pure value of type `A` and wrapping it in an `F`, producing an `F[A]`.
...
`pure` isn't a method defined on the `Int` type. It's a method defined on the `Applicative` trait. Scala offers the _implicit class_ construct that allows the compiler to perform conversions from the original type encountered in code (e.g. `Int`) to a type that supports whichever method that cannot be resolved during compilation (the `pure` method in our case).

This ability to attach methods onto various types is useful and can make working with **cats** type classes easier. In **cats** terminology, the collection of extension methods are called _syntax_.

Cats provides _syntax_ for all of its type classes.

Compare the two ways of working with the `Functor` type class:

{% highlight scala %}
Functor[Function0].map(myFun)(_.length)
myFun.map(_.length)
{% endhighlight %}
