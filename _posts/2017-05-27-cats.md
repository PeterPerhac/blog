...
Again compare the two ways of working with the `Functor` type class:


{% highlight scala %}
// choosing State monad, as many other monads already have a `map` method
Functor[State[Int, Int]].map(myState)(mappingFunction)
myState.map(mappingFunction)
{% endhighlight %}
